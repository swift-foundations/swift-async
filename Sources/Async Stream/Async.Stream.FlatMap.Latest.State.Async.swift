...
extension Async.Stream.FlatMap.Latest.State.Async {
    @usableFromInline
    func next() async -> U? {
        while true {
...
// MARK: - FlatMapLatest Method (async)
...
extension Async.Stream {
    ...
    public func flatMapLatest<U: Sendable>(
        ...
        Async.Stream<U> { [self] in
            let state: Self.FlatMap.Latest.State<U>.Async = .init(stream: self, transform: transform)
